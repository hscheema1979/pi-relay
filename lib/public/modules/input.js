...
export function autoResize() {
  ...
  // Ensure stop button shows during processing regardless of input content
  syncSendStopButton();
}
...
function syncSendStopButton() {
  ...
  if (ctx.processing) {
    ...
    ctx.sendBtn.classList.add("stop-mode");
  } else {
    ctx.sendBtn.classList.remove("stop-mode");
    ctx.sendBtn.disabled = !ctx.inputEl.value.trim() && pendingImages.length === 0 && pendingPastes.length === 0;
  }
}
...
export function initInput(_ctx) {
  ...
  document.addEventListener("paste", function (e) {
    // Don't intercept paste in modals or other input fields
    var active = document.activeElement;
    if (active && active !== ctx.inputEl && (active.tagName === "INPUT" || active.tagName === "TEXTAREA" || active.isContentEditable)) {
      return;
    }
  ...
  ctx.inputEl.addEventListener("keydown", function (e) {
    ...
    // Ctrl+J: insert newline (like pi CLI)
