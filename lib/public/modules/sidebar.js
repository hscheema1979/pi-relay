...
function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid) {
  ...
  if (cliSid) {
    ...
    copyResumeItem.addEventListener("click", function (e) {
      e.stopPropagation();
      copyToClipboard("pi --resume " + cliSid).then(function () {
  ...
  // Portal: append to body to escape #sidebar's overflow:hidden and sidebar stacking order.
  // Without this the menu is clipped by the sidebar and buried under #sidebar-footer.
  document.body.appendChild(menu);
  sessionCtxMenu = menu;
  refreshIcons();

  // Position using fixed coordinates so it sits above all sidebar chrome.
  requestAnimationFrame(function () {
    ...
    // Default: below the button, right-aligned to the button's right edge
...
export function updatePageTitle() {
  ...
  if (ctx.headerTitleEl) {
    ctx.headerTitleEl.textContent = sessionTitle || ctx.projectName || "Pi Relay";
  }
  if (ctx.projectName && sessionTitle) {
    document.title = sessionTitle + " - " + ctx.projectName;
  } else if (ctx.projectName) {
    document.title = ctx.projectName + " - Pi Relay";
  } else {
    document.title = "Pi Relay";
  }
}
...
export function populateCliSessionList(sessions) {
  ...
  for (var i = 0; i < sessions.length; i++) {
    ...
    (function (sessionId, firstPrompt) {
      item.addEventListener("click", function () {
        if (ctx.ws && ctx.connected) {
          ctx.ws.send(JSON.stringify({ type: "resume_session", cliSessionId: sessionId, firstPrompt: firstPrompt }));
        }
