...
export function showDoneNotification() {
  ...
  if (activeItem) {
    ...
    if (textEl) sessionTitle = textEl.textContent || "Pi";
    else sessionTitle = activeItem.textContent || "Pi";
  }
...
export function initNotifications(_ctx) {
  ...
  (function () {
    ...
    copyBtn.addEventListener("click", function () {
      copyToClipboard("npx pi-relay@latest").then(function () {
  ...
  onboardingDismissed = "1"; // All banners disabled
  ...
  if (location.protocol !== "https:" && location.hostname !== "localhost") {
    if (!onboardingDismissed) {
      showOnboarding(
        iconHtml("bell-ring") +
        ' Get alerts on your phone when Pi is done. <a href="/setup">Set up HTTPS</a>'
      );
    }
  }
  ...
  (function initServiceWorker() {
    ...
    navigator.serviceWorker.register((window._piRelayBase || "") + "/sw.js")
      ...
      debugToggleOnboarding.addEventListener("change", function () {
        if (debugToggleOnboarding.checked) {
          if (!onboardingText.innerHTML.trim()) {
            ...
            // onboardingBanner.classList.remove("hidden"); // All banners disabled
          }
        } else {
          hideOnboarding();
        }
