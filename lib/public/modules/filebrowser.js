...
export function initFileBrowser(_ctx) {
  ...
  // --- Edit / Save ---
  var editBtn = document.getElementById("file-viewer-edit");
  var saveBtn = document.getElementById("file-viewer-save");
  ...
  if (editorEl) {
    editorEl.addEventListener("input", function () {
      if (isEditing && editorEl.value !== currentContent) {
        isDirty = true;
  ...
  document.addEventListener("keydown", function (e) {
    if ((e.ctrlKey || e.metaKey) && e.key === "s") {
      if (isEditing && !ctx.fileViewerEl.classList.contains("hidden")) {
        e.preventDefault();
        saveFile();
      }
    }
  ...
  // ESC to close (exit edit mode first if editing)
  document.addEventListener("keydown", function (e) {
...
var cmView = null; // CodeMirror EditorView instance

function getLanguageExt(filePath) {
...
async function enterEditMode() {
  var wrapEl = document.getElementById("file-editor-wrap");
  var editBtn = document.getElementById("file-viewer-edit");
  var saveBtn = document.getElementById("file-viewer-save");
  if (!wrapEl || currentContent === null) return;

  isEditing = true;
  isDirty = false;
  ctx.fileViewerEl.classList.add("editing");
  ...
  try {
    var cm = await window._loadCodeMirror();

    // Pick language extension
    var langExtensions = [];
    var lang = getLanguageExt(currentFilePath);
    if (lang === "javascript" && cm.javascript) langExtensions.push(cm.javascript());
    else if (lang === "python" && cm.python) langExtensions.push(cm.python());
    else if (lang === "html" && cm.html) langExtensions.push(cm.html());
    else if (lang === "css" && cm.css) langExtensions.push(cm.css());
    else if (lang === "json" && cm.json) langExtensions.push(cm.json());
    else if (lang === "markdown" && cm.markdown) langExtensions.push(cm.markdown());

    var saveKeymap = cm.keymap.of([{
      key: "Mod-s",
      run: function () { saveFile(); return true; },
    }]);

    var updateListener = cm.EditorView.updateListener.of(function (update) {
      if (update.docChanged) {
        var newContent = update.state.doc.toString();
        isDirty = newContent !== currentContent;
        if (saveBtn) saveBtn.classList.toggle("unsaved", isDirty);
      }
    });
    ...
    wrapEl.innerHTML = '<div style="padding:20px;color:var(--error)">Failed to load editor: ' + e.message + '</div>';
  }
}
...
function exitEditMode() {
  ...
  isEditing = false;
  isDirty = false;
  ctx.fileViewerEl.classList.remove("editing");
...
function saveFile() {
  var saveBtn = document.getElementById("file-viewer-save");
  if (!cmView || !currentFilePath || !ctx.ws || !ctx.connected) return;
...
export function handleFsWrite(msg) {
  var saveBtn = document.getElementById("file-viewer-save");
  if (msg.ok) {
    if (saveBtn) {
      saveBtn.classList.add("edit-active");
      setTimeout(function () { saveBtn.classList.remove("edit-active"); }, 600);
    }
  } else {
...
function showFileContent(msg) {
  // Exit edit mode when switching to a new file
  if (isEditing && msg.path !== currentFilePath) exitEditMode();
  ...
  if (msg.error) {
    ...
    // Show edit button for text files
...
function renderHistoryPanel() {
  ...
  for (var i = 0; i < currentHistoryEntries.length; i++) {
    ...
    if (item.source === "git") {
      ...
      badge.textContent = item.toolName === "Write" ? "Pi Write" : "Pi Edit";
    }
