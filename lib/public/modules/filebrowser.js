...
function enterEditMode() {
  ...
  // Create or reuse Ace instance
  if (!aceEditor) {
    aceEditor = ace.edit(wrapEl);
    ...
    aceEditor.session.on("change", function () {
      if (!isEditing) return;
      isDirty = aceEditor.getValue() !== currentContent;
      if (saveBtn) saveBtn.classList.toggle("unsaved", isDirty);
    });
  }

  // Set content and mode
  var mode = getAceMode(currentFilePath);
  aceEditor.session.setMode("ace/mode/" + mode);
  aceEditor.setValue(currentContent, -1); // -1 = cursor at start
...
function saveFile() {
  var saveBtn = document.getElementById("file-viewer-save");
  if (!aceEditor || !currentFilePath || !ctx.ws || !ctx.connected) return;
