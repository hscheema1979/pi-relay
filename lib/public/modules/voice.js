...
// TTS engine fallback — auto-switch to native when server engine is down
var fallbackActive = false;    // True when we've fallen back to native for this response
var engineFailCount = 0;       // Consecutive server TTS failures in current response
var ENGINE_FAIL_THRESHOLD = 1; // Fall back after this many failures in a response
...
function updateOverlayEngineLabel() {
  ...
  if (engineLabel) {
    ...
    if (fallbackActive) {
      var nativeVoice = selectedVoices.native || "default";
      engineLabel.textContent = "Native (fallback) · " + nativeVoice + " · " + playbackSpeed.toFixed(1) + "×";
    } else {
...
function dispatchTTS(text) {
  ...
  if (ttsEngine === "native" || fallbackActive) {
...
function requestServerTTS(text) {
  ...
  .catch(function (err) {
    console.error("[voice] " + engineAtRequest + " TTS error:", err.message);
    engineFailCount++;
    if (engineFailCount >= ENGINE_FAIL_THRESHOLD && !fallbackActive) {
      fallbackActive = true;
      console.warn("[voice] " + engineAtRequest + " engine unavailable, falling back to native TTS");
      if (ctx.showToast) {
        ctx.showToast("TTS engine unavailable — using native voice", "warning");
      }
      updateOverlayEngineLabel();
    }
    // Speak the failed text via native so the user still hears it
    requestNativeTTS(text);
  })
  .finally(function () {
    ttsInFlight--;
