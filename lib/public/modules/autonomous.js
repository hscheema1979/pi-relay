...
// Debounce timer for loadAutonomousSessions
var debounceTimer = null;
var DEBOUNCE_MS = 500;     // collapse rapid WS events into a single fetch
var lastFetchTime = 0;     // timestamp of last completed fetch

// In-flight fetch tracking to avoid overlapping requests
var fetchInFlight = false;
var fetchPendingAfter = false;  // another fetch requested while one was in-flight
 ...
 * Debounced — rapid successive WS events are collapsed into a single fetch.
 */
...
function startBgPolling() {
  stopBgPolling();
  // Initial badge fetch (debounced so it doesn't duplicate if WS already triggered one)
  debouncedFetch();
...
// --- Data fetching with debounce and deduplication ---

/**
 * Debounced fetch — collapses rapid calls (e.g. multiple WS events) into one.
 */
...
/**
 * Actually perform the fetch. Prevents overlapping requests.
 */
function doFetch() {
  if (fetchInFlight) {
    fetchPendingAfter = true;
    return;
  }
  fetchInFlight = true;
...
function renderActiveCard(session) {
  ...
  if (session.sessionId && session.type !== "agent-fix") {
...
function switchToSession(sessionId) {
  ...
  if (ws && connected) {
    // Session IDs in the relay are numeric. Parse as integer when possible,
    // but preserve the original string if it's not a valid number (e.g. UUIDs).
    var parsed = parseInt(sessionId, 10);
    var id = (!isNaN(parsed) && String(parsed) === String(sessionId)) ? parsed : sessionId;
