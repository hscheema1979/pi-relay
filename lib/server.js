...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    if (req.method === "GET" && fullUrl === "/api/tts/voices") {
      ...
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ voices: tts.VOICES, default: tts.DEFAULT_VOICE }));
      return;
    }
    ...
    if (req.method === "POST" && fullUrl === "/api/tts/speak") {
      ...
      req.on("end", function () {
        try {
          ...
          var ttsOpts = {
            text: ttsData.text,
            voice: ttsData.voice || tts.DEFAULT_VOICE,
            model: ttsData.model || tts.DEFAULT_MODEL,
          };
          // Allow overriding LiteLLM connection from server config
          if (opts.litellmHost) ttsOpts.litellmHost = opts.litellmHost;
          if (opts.litellmPort) ttsOpts.litellmPort = opts.litellmPort;
          if (opts.litellmKey) ttsOpts.litellmKey = opts.litellmKey;

          tts.speak(ttsOpts).then(function (result) {
            res.writeHead(200, {
              "Content-Type": result.mimeType,
              "Content-Length": result.audio.length,
              "X-TTS-Duration-Ms": result.durationMs,
    ...
    if (req.method === "POST" && fullUrl === "/api/remote-projects") {
      ...
      req.on("end", function () {
        try {
          ...
          var slug = data.slug || (data.machineName ? data.machineName + "-" + data.remoteSlug : data.remoteSlug);
          ...
          var authCookie = req.headers.cookie || null;
  ...
  function addRemoteProject(opts) {
    var slug = opts.slug || (opts.machineName ? opts.machineName + "-" + opts.remoteSlug : opts.remoteSlug);
