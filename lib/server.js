...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    // --- TTS: synthesize speech (multi-engine) ---
    if (req.method === "POST" && fullUrl === "/api/tts/speak") {
      ...
      req.on("end", function () {
        try {
          ...
          var engine = ttsData.engine || "kokoro";
          var ttsOpts = {
            engine: engine,
            text: ttsData.text,
            voice: ttsData.voice,
          ...
          if (engine === "gemini") {
            ttsOpts.voice = ttsData.voice || tts.GEMINI_DEFAULT_VOICE;
            ttsOpts.model = ttsData.model || tts.GEMINI_DEFAULT_MODEL;
          ...
          // Kokoro-specific
          if (engine === "kokoro") {
            ttsOpts.voice = ttsData.voice || tts.KOKORO_DEFAULT_VOICE;
            if (opts.chutesApiKey) ttsOpts.chutesApiKey = opts.chutesApiKey;
          }
          // Edge-specific
          if (engine === "edge") {
            ttsOpts.voice = ttsData.voice || tts.EDGE_DEFAULT_VOICE;
          }
