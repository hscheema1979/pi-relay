...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    if (req.method === "DELETE" && fullUrl.match(/^\/api\/remote-projects\/[a-zA-Z0-9_-]+$/)) {
      if (!isAuthed(req, authToken)) {
        res.writeHead(401, { "Content-Type": "application/json" });
        res.end('{"error":"unauthorized"}');
        return;
  ...
  server.on("upgrade", function (req, socket, head) {
    ...
    if (origin) {
      try {
        ...
        // Extract host info from Host header for reverse proxy support.
        var hostHeader = req.headers.host || "";
        var hostHost, hostPort;
        try {
          var hostUrl = new URL(originUrl.protocol + "//" + hostHeader);
          hostHost = hostUrl.hostname;
          hostPort = String(hostUrl.port || (originUrl.protocol === "https:" ? "443" : "80"));
        } catch (e2) {
          hostHost = hostHeader.split(":")[0];
          hostPort = String(portNum);
        }
        // Allow if: origin port matches backend port, OR origin matches host header
        // (reverse proxy case: origin=https://domain:443, host=domain, backend=3015)
        var originMatchesHost = (originHost === hostHost && originPort === hostPort);
        var originMatchesBackend = (originPort === String(portNum));
        if (!originMatchesBackend && !originMatchesHost) {
