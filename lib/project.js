...
// Pi SDK is loaded lazily inside pi-bridge.js
...
function createProjectContext(opts) {
  ...
  var autonomousBroadcastInterval = setInterval(function () {
    ...
    try {
      ...
      if (hash !== lastAutonomousHash) {
        ...
        send({ type: "autonomous_update", data: state });
      }
    } catch (e) {}
  }, 10000);
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "autonomous_action") {
      ...
      var targetSession = sm.sessions.get(targetSid) || sm.sessions.get(parseInt(targetSid, 10));
      if (targetSession) {
  ...
  function handleHTTP(req, res, urlPath) {
    ...
    if (req.method === "GET" && urlPath === "/api/autonomous-sessions") {
      ...
      try {
        ...
        if (fs.existsSync(fleetStatusFile)) {
          var fleetStatus = JSON.parse(fs.readFileSync(fleetStatusFile, "utf8"));
        ...
        // Fleet status file not available — that's fine
      }
