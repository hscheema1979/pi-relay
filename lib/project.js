...
var { createFleetBridge } = require("./fleet-bridge");
...
function createProjectContext(opts) {
  ...
  // --- Pi bridge ---
  // fleetBridgeRef is set after fleet-bridge is created (deferred binding).
  // This avoids a circular dependency: bridge needs fleetBridge for hooks,
  // but fleetBridge needs bridge for agent queries.
  var fleetBridgeRef = null;
  ...
  var bridge = createPiBridge({
    ...
    debug: debug,
    onToolEvent: function (event, toolName, toolId, input) {
      if (fleetBridgeRef) {
        fleetBridgeRef.fireHook(event, toolName, { toolId: toolId, input: input });
      }
    },
  });

  // --- Fleet bridge (orchestration: loops, schedules, fleet, autoresearch) ---
  var fleetBridge = createFleetBridge({
    cwd: cwd,
    slug: slug,
    send: send,
    sendTo: sendTo,
  ...
  fleetBridgeRef = fleetBridge;  // complete the deferred binding
  ...
  function handleMessage(ws, msg) {
    // Fleet orchestration messages (loop_*, schedule_*, fleet_*, task_*, etc.)
    if (fleetBridge.handleMessage(ws, msg)) return;
  ...
  function handleHTTP(req, res, urlPath) {
    ...
    // Orchestrator API endpoints (fleet-bridge data for frontend panels)
    if (req.method === "GET" && urlPath.startsWith("/api/orchestrator/")) {
      var module = urlPath.replace("/api/orchestrator/", "").split("?")[0];
      var data = fleetBridge.getOrchestratorData(module);
      res.writeHead(200, { "Content-Type": "application/json" });
  ...
  function destroy() {
    ...
    stopFileWatch();
    stopAllDirWatches();
    // Cleanup fleet bridge (orchestrator + remote connections)
    fleetBridge.shutdown();
    // Cleanup pi bridge
    bridge.destroy();
    // Kill all terminals
    tm.destroyAll();
