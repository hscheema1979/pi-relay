...
async function discover(opts) {
  ...
  // Probe online peers for pi-relay in parallel
  var probePromises = [];
  for (var j = 0; j < peers.length; j++) {
    ...
    probePromises.push((function (peer) {
      ...
      return probeRelay(probeHost, relayPort, probeTimeout).then(function (info) {
        if (info && (info.projects || info.version)) {
          peer.hasRelay = true;
          peer.relay = info;
        } else {
          peer.hasRelay = false;
