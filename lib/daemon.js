        ...
        if (interrupted.length > 0) {
          ...
          for (var ii = 0; ii < interrupted.length; ii++) {
            ...
            console.log("[daemon]   - " + (s.type || "unknown") + " session " + s.sessionId +
              (s.plan ? " (phase " + s.plan.currentPhase + "/" + s.plan.phases.length + ")" : ""));
          }
          ...
          (function (interruptedSessions) {
            setTimeout(function () {
              ...
              for (var j = 0; j < interruptedSessions.length; j++) {
                ...
                var desc = sess.type === "agent-fix"
                  ? "Agent fix for " + (sess.service || "unknown") + " @ " + (sess.machine || "unknown")
                  : "Execution plan" + (sess.plan ? " (phase " + sess.plan.currentPhase + "/" + sess.plan.phases.length + ")" : "");
                messages.push(desc);
              }
              relay.broadcastAll({
                type: "toast",
