    ...
    upstream.on("message", function (data) {
      ...
      try {
        ...
        if (!initPhaseComplete) {
          ...
          // After init phase, update cached state so late-joining clients get current state.
          // We track a post-init history phase to capture the latest session's history.
          if (parsed.type === "history_meta") {
            // New session switched — reset history cache and start tracking
            inHistoryPhase = true;
            cachedHistoryEntries = [];
          }

          if (cacheableTypes[parsed.type]) {
            // Replace any previous cached version of this message type
            var replaced2 = false;
            ...
            if (!replaced2) cachedInitMessages.push(parsed);
          } else if (inHistoryPhase && parsed.type !== "history_done") {
            // Cache history content entries for the current session
            cachedHistoryEntries.push(parsed);
          }
