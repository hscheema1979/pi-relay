  ...
  function _handlePiEvent(session, event) {
    switch (event.type) {
      ...
      case "agent_end":
        ...
        // Check execution mode sidecar on every agent_end
        if (autonomous) {
          var sidecar = readExecutionModeSidecar();
          if (sidecar) {
            // Detect mid-session plan activation: sidecar is active but session not yet tracked
            ...
            // Update plan progress in autonomous manager while active
            if (session.executionMode && (sidecar.status === "active" || sidecar.status === "running")) {
  ...
  async function startQuery(session, text, images) {
    try {
      ...
      if (state.piSession.isStreaming) {
        // Extension commands (e.g. /exec pause) must go through prompt() even
        // while streaming — steer() rejects them with "cannot be queued".
        // prompt() handles extension commands immediately regardless of streaming state.
        if (text.startsWith("/")) {
