...
<div class="step-card" id="step-tailscale">
  ...
  <div class="step-desc">Tailscale creates a private VPN so you can access Pi Relay from anywhere. It needs to be installed on <b>both</b> the server (the machine running Pi Relay) and this device.</div>

  <div class="instruction"><div class="inst-num">1</div>
    <div class="inst-text"><b>Server:</b> Install Tailscale on the machine running Pi Relay.
...
<div class="step-card" id="step-pwa">
  ...
  <div class="step-desc">Install Pi Relay as an app for quick access and a full-screen experience.</div>
  ...
  <div class="platform-desktop">
    ...
    <div class="instruction"><div class="inst-num">2</div>
      <div class="inst-text">Click it and then click <b>Install</b> to confirm.
        <div class="note">If there is no icon, go to <b>Menu > Install Pi Relay</b> or <b>Menu > Save and Share > Install</b>.</div>
      </div>
    </div>
  </div>

  <div id="pwa-status" class="check-status pending">After installing, open Pi Relay from your home screen to continue setup.</div>
...
<div class="step-card" id="step-push">
  ...
  <div class="step-desc">Get alerted on your phone when Pi finishes a response, even when the app is in the background.</div>
...
<div class="step-card" id="step-done">
  <div class="done-card">
    ...
    <a class="btn" id="done-link" href="${httpsUrl}">Open Pi Relay</a>
  </div>
</div>
</div>
...
function pushDone() {
  ...
  navigator.serviceWorker.ready.then(function(reg) {
    reg.showNotification("\ud83c\udf89 Welcome to Pi Relay!", {
      body: "\ud83d\udd14 You\u2019ll be notified when Pi responds.",
      tag: "pi-welcome",
    });
  }).catch(function() {});
...
function dashboardPageHtml(projects, version, machines) {
  // Split local and remote projects
  var localProjects = [];
  var remoteProjectsList = [];
  for (var i = 0; i < projects.length; i++) {
    if (projects[i].remote) remoteProjectsList.push(projects[i]);
    else localProjects.push(projects[i]);
  }

  // Local project cards
  var localCards = "";
  for (var i = 0; i < localProjects.length; i++) {
    var p = localProjects[i];
    ...
    localCards += '<a class="card" href="/p/' + p.slug + '/">' +
  ...
  // Remote project cards (already connected)
  var remoteCards = "";
  for (var i = 0; i < remoteProjectsList.length; i++) {
    var rp = remoteProjectsList[i];
    var rpDisplayName = rp.title || rp.project;
    var rpPathDisplay = rp.remoteHost + ":" + rp.remotePort + "/" + rp.remoteSlug;
    remoteCards += '<a class="card card-is-remote" href="/p/' + rp.slug + '/">' +
      '<div class="card-title">' + escapeHtml(rpDisplayName) + ' <span class="card-remote">📍 ' + escapeHtml(rp.machine || "remote") + '</span></div>' +
      '<div class="card-path">' + escapeHtml(rpPathDisplay) + '</div>' +
      '</a>';
  }

  // Machine cards (from tailscale discovery)
  var machineCards = "";
  var relayMachines = (machines || []).filter(function (m) { return m.hasRelay; });
  for (var i = 0; i < relayMachines.length; i++) {
    var m = relayMachines[i];
    var mProjects = (m.relay && m.relay.projects) || [];
    var projCount = mProjects.length;
    var projLabel = projCount === 1 ? "1 project" : projCount + " projects";
    var statusClass = m.online ? "online" : "offline";
    machineCards += '<a class="card card-machine" href="/m/' + escapeHtml(m.hostname) + '/">' +
      '<div class="card-title"><span class="machine-dot ' + statusClass + '"></span> ' + escapeHtml(m.hostname) + '</div>' +
      '<div class="card-path">' + escapeHtml(m.ip || m.fqdn) + (m.relayPort ? ':' + m.relayPort : '') + '</div>' +
      '<div class="card-meta">' + projLabel + (m.online ? '' : ' · offline') + '</div>' +
      '</a>';
  }

  if (localProjects.length === 0 && remoteProjectsList.length === 0 && relayMachines.length === 0) {
    localCards = '<div class="empty">No projects registered. Run <code>pi-relay</code> in a project directory to add one.</div>';
  }

  var sectionsHtml = '';
  if (localCards || remoteCards) {
    sectionsHtml += '<div class="section-label">Projects</div>';
    sectionsHtml += '<div class="cards">' + localCards + remoteCards + '</div>';
  }
  if (machineCards) {
    sectionsHtml += '<div class="section-label" style="margin-top:24px">Machines</div>';
    sectionsHtml += '<div class="cards">' + machineCards + '</div>';
  }
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    ...
    '.subtitle{font-size:13px;color:#8B887F;margin-bottom:24px}' +
    '.section-label{font-size:11px;font-weight:600;color:#8B887F;text-transform:uppercase;letter-spacing:0.5px;padding:0 4px 6px;width:100%;max-width:480px}' +
    ...
    '.card-remote{font-size:11px;color:#8B887F;margin-left:8px}' +
    '.card-is-remote{border-left:3px solid #6B9BD2}' +
    '.card-machine{border-left:3px solid #57AB5A}' +
    '.card-machine:hover{border-color:#57AB5A}' +
    '.machine-dot{display:inline-block;width:8px;height:8px;border-radius:50%;flex-shrink:0}' +
    '.machine-dot.online{background:#57AB5A}' +
    '.machine-dot.offline{background:#6B6862}' +
    ...
    '</style></head><body>' +
    '<h1>Pi Relay</h1>' +
    '<div class="subtitle">Select a project or machine</div>' +
    sectionsHtml +
...
function machinePageHtml(machineName, machineInfo, version) {
  var projects = (machineInfo && machineInfo.projects) || [];
  var ip = machineInfo ? (machineInfo.ip || '') : '';
  var port = machineInfo ? (machineInfo.port || 3002) : 3002;
  var isOnline = machineInfo && machineInfo.online;

  var cards = "";
  for (var i = 0; i < projects.length; i++) {
    var p = projects[i];
    var slug = p.slug || p.project;
    var displayName = p.project || slug;
    cards += '<a class="card" href="/p/' + escapeHtml(slug) + '/">' +
      '<div class="card-title">' + escapeHtml(displayName) + '</div>' +
      '<div class="card-path">' + escapeHtml(slug) + '</div>' +
      '</a>';
  }

  if (!isOnline) {
    cards = '<div class="empty offline-msg">⚠️ Machine <strong>' + escapeHtml(machineName) + '</strong> is currently offline or unreachable.</div>';
  } else if (projects.length === 0) {
    cards = '<div class="empty">No projects found on this machine.</div>';
  }

  return '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    '<title>' + escapeHtml(machineName) + ' - Pi Relay</title>' +
    '<link rel="icon" href="/favicon.svg" type="image/svg+xml">' +
    '<style>' +
    '*{margin:0;padding:0;box-sizing:border-box}' +
    'body{background:#2F2E2B;color:#E8E5DE;font-family:-apple-system,system-ui,sans-serif;min-height:100vh;display:flex;flex-direction:column;align-items:center;padding:40px 20px}' +
    'h1{font-size:24px;font-weight:600;margin-bottom:4px;color:#E8E5DE;display:flex;align-items:center;gap:10px}' +
    '.machine-dot{display:inline-block;width:10px;height:10px;border-radius:50%;flex-shrink:0}' +
    '.machine-dot.online{background:#57AB5A}' +
    '.machine-dot.offline{background:#6B6862}' +
    '.subtitle{font-size:13px;color:#8B887F;margin-bottom:16px}' +
    '.back{display:inline-flex;align-items:center;gap:6px;color:#8B887F;text-decoration:none;font-size:13px;margin-bottom:20px;transition:color .15s}' +
    '.back:hover{color:#E8E5DE}' +
    '.back svg{width:16px;height:16px}' +
    '.search-wrap{width:100%;max-width:480px;margin-bottom:16px}' +
    '.search-wrap input{width:100%;background:#3A3936;border:1px solid #4A4845;border-radius:10px;color:#E8E5DE;font-size:14px;padding:10px 14px;outline:none;font-family:inherit;transition:border-color .15s}' +
    '.search-wrap input:focus{border-color:#DA7756}' +
    '.search-wrap input::placeholder{color:#6B6862}' +
    '.cards{display:flex;flex-direction:column;gap:12px;width:100%;max-width:480px}' +
    '.card{display:block;background:#3A3936;border:1px solid #4A4845;border-radius:12px;padding:16px 20px;text-decoration:none;color:#E8E5DE;transition:border-color .15s,background .15s}' +
    '.card:hover{border-color:#DA7756;background:#3F3D3A}' +
    '.card-title{font-size:16px;font-weight:600;display:flex;align-items:center;gap:8px}' +
    '.card-path{font-size:12px;color:#8B887F;margin-top:4px;font-family:monospace}' +
    '.card.hidden-by-search{display:none}' +
    '.empty{text-align:center;color:#6B6862;font-size:14px;padding:40px 20px}' +
    '.offline-msg{color:#DA7756;background:#3A3936;border:1px solid #4A4845;border-radius:12px;padding:20px}' +
    '.offline-msg strong{color:#E8E5DE}' +
    '.footer{margin-top:40px;font-size:11px;color:#4A4845}' +
    '</style></head><body>' +
    '<a class="back" href="/">' +
    ...
    'All projects</a>' +
    '<h1><span class="machine-dot ' + (isOnline ? 'online' : 'offline') + '"></span> ' + escapeHtml(machineName) + '</h1>' +
    '<div class="subtitle">' + escapeHtml(ip + (port ? ':' + port : '')) + '</div>' +
    (isOnline && projects.length > 1 ? '<div class="search-wrap"><input id="project-search" type="text" placeholder="Filter projects..." autocomplete="off" spellcheck="false"></div>' : '') +
    '<div class="cards" id="project-cards">' + cards + '</div>' +
    '<div class="footer">v' + escapeHtml(version || "") + '</div>' +
    '<script>' +
    'var si=document.getElementById("project-search");' +
    ...
    'cards[i].classList.toggle("hidden-by-search",q&&t.indexOf(q)===-1);}});}' +
    '</script>' +
    '</body></html>';
}
...
module.exports = { pinPageHtml, setupPageHtml, dashboardPageHtml, machinePageHtml };
