...
function dashboardPageHtml(projects, version) {
  ...
  for (var i = 0; i < projects.length; i++) {
    ...
    var remoteLabel = p.remote ? '<span class="card-remote">📍 ' + escapeHtml(p.machine || "remote") + '</span>' : "";
    var pathDisplay = p.remote ? (p.remoteHost + ":" + p.remotePort + "/" + p.remoteSlug) : p.path;
    cards += '<a class="card' + (p.remote ? " card-is-remote" : "") + '" href="/p/' + p.slug + '/">' +
      '<div class="card-title">' + escapeHtml(displayName) + ' <span class="card-status">' + statusIcon + '</span> ' + remoteLabel + '</div>' +
      '<div class="card-path">' + escapeHtml(pathDisplay) + '</div>' +
      '<div class="card-meta">' + sessionLabel + (p.remote ? '' : ' · ' + p.clients + ' client' + (p.clients !== 1 ? 's' : '')) + '</div>' +
      '</a>';
  }
  if (projects.length === 0) {
    cards = '<div class="empty">No projects registered. Run <code>pi-relay</code> in a project directory to add one.</div>';
  }
